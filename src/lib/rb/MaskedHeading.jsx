/* MaskedHeading — React Bits (github.com/DavidHDev/react-bits), MIT.

   Uma alteração em relação ao original, e só uma: o teto do corpo passou
   de 200px para 460px na linha do clamp abaixo. O componente calcula
   font-size como fração da largura do contêiner e limita em 200px, o que
   serve para uma manchete de seção — mas trava uma palavra de capa, que
   precisa ocupar a coluna inteira. A lógica é a mesma; só o teto mudou.
*/
...
const MaskedHeading = ({
  ...
  const place = useCallback(() => {
    ...
    /* MUDANÇA em relação ao original: o filtro só é reescrito quando muda.

       O componente reatribuía `media.style.filter` a cada quadro, com uma
       string sempre idêntica. Reatribuir um filtro força o navegador a
       reavaliar a cadeia inteira e re-rasterizar a camada, e o resultado
       era a cintilação — em Chrome e Safari — e os fiapos de borda ao
       passar o mouse. O transform continua por quadro; ele é barato e é
       o que de fato anima. */
    const filtro = `brightness(${s.brightness}) saturate(${s.saturation})${s.grayscale ? ' grayscale(1)' : ''}`;
    if (media.dataset.filtro !== filtro) {
      media.style.filter = filtro;
      media.dataset.filtro = filtro;
    }
  }, []);
  ...
  const sync = useCallback(() => {
    ...
    root.style.fontSize = `${clamp(root.clientWidth * s.textScale, 20, 460).toFixed(1)}px`;
